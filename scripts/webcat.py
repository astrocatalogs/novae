...
titles = [
    ...
    "Host Name",
    "Nova J2000 Right Ascension (h:m:s)",
    "Nova J2000 Declination (d:m:s)",
...
for fcnt, eventfile in enumerate(tq(sorted(files, key=lambda s: s.lower()))):
    ...
    if 'ra' in catalog[entry] and 'dec' in catalog[entry] and args.collecthosts:
        ...
        try:
            ...
            if eventname in hostimgdict:
                ...
                if hasimage and filecmp.cmp(outdir + htmldir + fileeventname + '-host.jpg', 'astrocats/novae/input/missing.jpg'):
                    hasimage = False
    ...
    if dohtml and args.writehtml:
        ...
        html = re.sub(r'(\<\/title\>)', r'''\1\n
            <base target="_parent" />\n
            <link rel="stylesheet" href="https://sne.space/astrocats/astrocats/novae/html/event.css" type="text/css">\n
            <script type="text/javascript" src="https://sne.space/astrocats/astrocats/novae/scripts/marks.js" type="text/css"></script>\n
        ...
        html = re.sub(r'(\<\/body\>)', '<div class="event-issue">' + r'<a href="https://github.com/astrocatalogs/novae/issues/new' +
                    ...
                    for r, row in enumerate(catalog[entry][key]):
                        if 'value' in row and 'source' in row:
                            ...
                            edit = "true" if os.path.isfile(
                                'astrocats/novae/input/sne-internal/' + get_event_filename(entry) + '.json') else "false"
